...
class Browser:
    def __init__(self, root):
        self.root = root
        self.root.title("Mini Browser – Step 1")
        self.current_html = ""
        ...
        self.source_text = tk.Text(root, height=12)
    ...
    def load_url(self, event=None):
        ...
        if not url:
            ...
            if not path: return
            url = path
            self.url_entry.delete(0, tk.END); self.url_entry.insert(0, path)
        ...
        self.source_text.insert(tk.END, self.current_html)
        self.canvas.delete("all")  # no rendering yet
