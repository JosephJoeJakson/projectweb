...
def dom_to_string(node, indent=0):
    if not node: return ""
    pad = "  " * indent
    attr = " " + " ".join(f'{k}="{v}"' for k, v in node.attributes.items()) if node.attributes else ""
    out = f"{pad}<{node.tag}{attr}>"
    if node.text.strip(): out += " " + node.text.strip()
    lines = [out]
    for c in node.children: lines.append(dom_to_string(c, indent+1))
